...
int openAudioStream(AudioStream *audioStream, PaStreamCallback *streamCallback, void *userData){
  ...
  memset(&audioStream->input, 0, sizeof(PaStreamParameters));
  audioStream->input.channelCount = kNumChannels;
	audioStream->input.device = kAudioInputDeviceIndex;
	audioStream->input.sampleFormat = paFloat32;
	audioStream->input.suggestedLatency = 0.0;
